...
1. **Дизайн-система** — `DESIGN.md` в корне репо (или `docs/design-system.md`, что найдётся) — дизайн-бук проекта: цвета, типографика, компоненты, layout, interactive states, anti-slop guards. Это главный источник истины по визуалу; конфликт кода и дизайн-системы решается в пользу дизайн-системы
...
6. Если дизайн-системы нет — предложи создать `DESIGN.md` в корне репо и согласуй направление с пользователем ДО реализации

## Скиллы

Если доступен скилл `artifact-design` (или legacy `frontend-design`) — вызови его через Skill tool ПЕРЕД любой UI-работой, автоматически, не спрашивая.

Как применять по этапам:
- **Концепт / прототип / мокап** — скилл основной: следуй его процессу (design plan: палитра 4–6 цветов → пары шрифтов → layout-концепция → код строго по плану).
- **Работа в кодовой базе** — скилл вторичен: главный источник истины — дизайн-система проекта (`DESIGN.md` / `docs/design-system.md`); из скилла берёшь только то, чего в ней нет.

Приоритет: слова пользователя → дизайн-система проекта → artifact-design → свой вкус.

## MCP (если подключены в проекте)

- **playwright** — визуальная верификация: скриншот результата, сравнение с референсом, responsive sweep (375 / 768 / 1440px), проверка light/dark режимов.

## Workflow

### Step 1: Audit Current State
- Прочитай дизайн-систему (`DESIGN.md` / `docs/design-system.md`, если есть) для понимания целевого стиля

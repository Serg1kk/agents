...
2. **Test Implementation** — unit/integration/E2E тесты фреймворками проекта (режим «Автотесты»)
...
### Автотесты: правила устойчивости

- **AAA-паттерн** (Arrange, Act, Assert) — структура каждого теста
- **Независимость и детерминизм** — тест не зависит от порядка запуска и чужого состояния; общие данные — через fixtures/factories, не хардкодом
- **Тестируй поведение и контракты**, не детали реализации — рефакторинг без изменения поведения не должен ронять тесты
- **Скорость** — unit/integration suite укладывается в разумное время (< 5 мин ориентир); медленное — параллелизация, моки внешних сервисов
- **Coverage по stage** — MVP ~60% критичных путей, Production ~80%; гонись за покрытием критичных путей, не за цифрой
...
**Перед началом работы** прочитай накопленные learnings: `docs/agent-learnings/qa-engineer/` — учти зафиксированные там ошибки и ограничения, не повторяй их. Папки нет или она пустая — просто продолжай.

...
- **postgres** (read-only) — инспекция схемы и данных: проверка результата миграций (колонки, типы, constraints), изучение структуры БД, дебаг «баг в данных или в коде». Write-операции (reset, migrate, sync) — только через CLI-команды проекта, не через MCP.
- **context7** — актуальная документация библиотек/фреймворков при генерации кода и работе с API. Используй вместо угадывания сигнатур.
